# ⚡ Wi-Fi Cyber Suite | Advanced RF Sensing, 3D Threat Globe & Biometrics
...
> 
> A high-performance Wi-Fi surveillance, security audit, 360° radar, spectrum analyzer, **2D/3D RF Heatmap & Raytracer**, **Spy Cam & Drone Hunter**, **Mr. Robot OSINT Recon Hunter**, **fsociety Interactive Terminal Simulator**, **JARVIS Cyber Biometric rPPG Heart-Rate HUD**, and **3D Interactive Cyber Attack Global Threat Map & Warfare Globe**.
...
|---|---|---|
| **`RUN_WIFI_SUITE.bat`** | ⚡ **Master Command Center (`hub.py`)** | Central 8-module cyber dashboard to launch any module with 1 click |
| **`run_cyber_globe.bat`** | 🌐 **3D Cyber Threat Globe (`cyber_globe.py`)** | Interactive 3D vector globe, ballistic cyber missiles & DEFCON war room |
...
### 1. 🌐 3D Cyber Attack Global Threat Map & Warfare Globe (`cyber_globe.py`)
- **Interactive 3D Vector Globe:** 360° rotation with mouse dragging, scroll-wheel zoom, and smooth orbital auto-spin.
- **Ballistic 3D Laser Arc Missiles:** Real-time parabolic Bézier trajectories soaring into space with color-coded attack types (DDoS, LockBit Ransomware, Zero-Day RCE, SQLi Exfiltration, Botnet Relays).
...
- **War Room Telemetry:** Live streaming terminal feed and global attack bandwidth velocity gauge.

### 2. 🫀 Cyber Biometric rPPG Heart-Rate HUD (`cyber_biometrics.py`)
...
### 3. 🚨 Wi-Fi Motion Sentinel (`wifi_motion_sensor.py`)
...
### 4. 📡 Wi-Fi Radar & Security Audit (`wifi_radar.py`)
...
### 5. 🗺️ Wi-Fi 2D/3D Heatmap & RF Raytracer (`wifi_heatmap.py`)
...
### 6. 🕵️‍♂️ Spy Cam & Drone RF Hunter (`wifi_spy_hunter.py`)
...
### 7. 🔍 Elliot OSINT & Digital Footprint Hunter (`elliot_recon.py`)
...
### 8. 💀 fsociety Interactive Hack Terminal (`fsociety_terminal.py`)
